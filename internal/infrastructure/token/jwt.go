...
func (j *JWTManager) StoreRefreshToken(ctx context.Context, redisClient *redis.Client, userID, tokenID string, expiry time.Duration) error {
	key := fmt.Sprintf("refresh_token:%s:%s", userID, tokenID)
	return redisClient.SetWithTTL(ctx, key, "1", expiry)
}

func (j *JWTManager) IsRefreshTokenValid(ctx context.Context, redisClient *redis.Client, userID, tokenID string) (bool, error) {
	key := fmt.Sprintf("refresh_token:%s:%s", userID, tokenID)
	return redisClient.Exists(ctx, key)
}

func (j *JWTManager) RevokeRefreshToken(ctx context.Context, redisClient *redis.Client, userID, tokenID string) error {
	key := fmt.Sprintf("refresh_token:%s:%s", userID, tokenID)
	return redisClient.DeleteKey(ctx, key)
}